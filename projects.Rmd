...
``` 
  ...
  -  Please find the details of the Python code used  [here](https://github.com/punitjha/YOLO-Object-Detection). 
  -  All calculations were run on Google Colab. Due to limitations in availability of computational resources, the model could be trained only for 22 epochs. Some results are shown below.
...
<hr style="border:2px solid gray"> </hr>
```{r, echo=FALSE, out.width=" 45%",out.height="45%",fig.cap="Visualizing  the performance of our RNN  model by creating a confusion matrix. The ground truth languages of samples are represented by rows in the matrix while the predicted languages are represented by columns.",fig.show='hold',fig.align='left'}
knitr::include_graphics(c("confusion.png"))
```

  - Trained an RNN for two tasks on text data:
    - Language classification - RNN will learn to detect which language a chunk of text is written in (similar to a feature you might find in an online translator). While this might be relatively easy to do if the input text is Unicode and unique characters indicate a particular language, however this implementation addressed the case where all input text were converted to ASCII characters so our network must learn instead to detect letter patterns. The model was trained on the Bible which is a large text translated to different languages but in the same easily parsable format, so I used 20 different copies of the Bible in different languages obtained from Project Gutenberg.
...
```
  ...
  - In the second part, designed and train my own deep simple convolution network to predict a binary present/absent image-level label for each of the 20 PASCAL classes using AlexNet as the starting point. The goal was to get an average mAP score of at least 40 percent. 
...
```
  - Implemented multi-layer neural networks from scratch on the [<span style="color:blue"> CIFAR-10 </span>](https://www.cs.toronto.edu/~kriz/cifar.html) image classification data set, to understand the fundamentals of neural networks and backpropagation.
...
- Implemented a few ML algorithms from scratch using only Python Numpy. All of these algorithms are run on tiny data sets on a standard CPU laptop. These were part of assignments from CS-446 and CS-498.
